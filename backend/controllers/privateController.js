...
exports.toggleActiveInstructor = async (req, res) => {
  try {
    ...
    await instructor.save();
    res.json({
      message: `Đã ${instructor.is_active ? "Hiện" : "Ẩn"} giảng viên`,
    });
  } catch (err) {
    res.status(500).json({ message: "Server Error" });
  }
...
exports.updateCourse = async (req, res) => {
  ...
  try {
    ...
    if (title !== undefined && !title.trim()) {
      return res
        .status(400)
        .json({ message: "Tên khóa học không được để trống!" });
    }
    if (course_code !== undefined && !course_code.trim()) {
      return res
        .status(400)
        .json({ message: "Mã khóa học không được để trống!" });
    }
    if (sumary !== undefined && !sumary.trim()) {
